defmodule MoneyButton.WebhookTest do
  ...
  def parse(conn, opts \\ []) do
    opts =
      opts
      |> Keyword.put_new(:parsers, [MoneyButton.Webhook])
  ...
  test "unrelated json payload" do
    conn = Jason.encode!(%{"foo" => "bar"}) |> json_conn() |> parse(pass: ["*/*"])
    assert conn.params == %{}
  ...
  test "unrelated json payload - strict mode" do
    conn = Jason.encode!(%{"foo" => "bar"}) |> json_conn() |> parse(strict: true)
    assert conn.status == 403
    assert conn.resp_body == "Forbidden!"
    assert conn.state == :sent
  end

  test "unrelated content type" do
    conn = "<html></html>" |> json_conn("text/html") |> parse(pass: ["*/*"])
    assert conn.params == %{}

    {:ok, raw_body, _} = Plug.Conn.read_body(conn)
    assert raw_body == "<html></html>"
  end
end
